...
const listarFilmes = (filmes) => {
    ...
    filmes.forEach(filme => {
        ...
        h5.textContent = filme.titulo;
        cardBody.appendChild(h5);

        let aluguel = document.createElement('p');
        aluguel.classList.add('card-text');
        aluguel.innerHTML = `<strong>Valor:</strong> R$ ${filme.aluguel.toFixed(2)}`;
        cardBody.appendChild(aluguel);
        ...
        ano.innerHTML = `<strong>Data de Lançamento:</strong> ${filme.ano}`;
        ...
        icon1.classList.add('btn', 'btn-success');
        icon1.innerHTML = `
            ...
            <span><span id="like-count-${filme.id}">0</span> Gostei</span>
        `;
        divButtonGroup.appendChild(icon1);

        let icon2 = document.createElement('button');
        icon2.classList.add('btn', 'btn-danger');
        icon2.innerHTML = `
            ...
            <span><span id="dislike-count-${filme.id}">0</span> Não Gostei</span>
        `;
        divButtonGroup.appendChild(icon2);

        listarFtable.appendChild(col);

        icon1.addEventListener('click', () => {
            likeCount++;
            document.getElementById(`like-count-${filme.id}`).textContent = likeCount;
        });

        icon2.addEventListener('click', () => {
            dislikeCount++;
            document.getElementById(`dislike-count-${filme.id}`).textContent = dislikeCount;
        });
    });
};
